...
class SelectConstructor {
	...
	getOptions(originalSelect) {
		...
		let selectOptionsScrollHeight = originalSelect.dataset.scroll ? `style="max-height:${originalSelect.dataset.scroll}px"` : `style="max-height:200px"`;
		...
		if (selectOptions.length > 0) {
			...
			selectOptionsHTML += selectOptionsScroll ? `<div ${selectOptionsScroll} ${selectOptionsScrollHeight} class="${this.selectClasses.classSelectOptionsScroll}">` : `<div class="${this.selectClasses.classSelectOptionsScroll}">`;
	...
	setOptionsPosition(selectItem) {
		...
		const selectOptionsPosMargin = +originalSelect.dataset.optionsMargin ? +originalSelect.dataset.optionsMargin : 0;
		...
		if (!selectItem.classList.contains(this.selectClasses.classSelectOpen)) {
			...
			const selectItemTotal = selectItemPos + selectOptionsHeight + selectItemHeight + (selectOptionsHeight - selectItemScrollHeight);
			const selectItemResult = window.innerHeight - (selectItemTotal + selectOptionsPosMargin);

			if (selectItemResult < 0) {
				const newMaxHeightValue = selectOptionsHeight + selectItemResult;
				if (newMaxHeightValue < 70) {
					selectItem.classList.add('select_show-top');
					selectItemScroll.style.maxHeight = customMaxHeightValue ? `${customMaxHeightValue}px` : `200px`;
				} else {
			...
			setTimeout(() => {
				selectItem.classList.remove('select_show-top');
				if (originalSelect.hasAttribute('data-scroll')) {
					selectItemScroll.style.maxHeight = customMaxHeightValue ? `${customMaxHeightValue}px` : `200px`;
				} else {
