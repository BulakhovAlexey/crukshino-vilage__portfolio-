...
// Модуль прокрутки, чтобы заблокировать
...
// Работа с полями формы.Добавление занятий, работа с заполнителем
export function formFieldsInit(
  options = { viewPass: false, autoHeight: false }
) {
  // Если включено, добавьте функциональность «скрыть Pleischolder в фокусе»
  ...
  // Если включено, добавьте функциональность «Показать пароль»
  if (options.viewPass) {
  ...
  // Если включено, мы добавляем функциональность «Automox»
...
export function formSubmit() {
  ...
  function formSent(form, responseResult = ``) {
    // Мы создаем событие отправки формы
    ...
    //Показать Popap, если модуль подключен
    // и для указанных настроек формы
    setTimeout(() => {
      if (flsModules.popup) {
...
export function formRating() {
  ...
  function initRatings() {
    ...
    // «Запустить» на всех рейтингах на странице
    for (let index = 0; index < ratings.length; index++) {
      const rating = ratings[index]
      initRating(rating)
    }
    // Инициализировать конкретный рейтинг
    ...
    // Мы меняем ширину активных звезд
    ...
    async function setRatingValue(value, rating) {
      if (!rating.classList.contains("rating_sending")) {
        ...
        // Отправка данных (значение) на сервер
        ...
        if (response.ok) {
          ...
          // Мы получаем новый рейтинг
          const newRating = result.newRating

          // Отмена нового среднего результата
